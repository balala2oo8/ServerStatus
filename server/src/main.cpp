...
int CMain::HandleMessage(int ClientNetID, char *pMessage)
{
	...
	if(str_comp_num(pMessage, "update", sizeof("update")-1) == 0)
	{
		...
		// 记录最近一次成功上报数据的时间，用于页面展示
		pClient->m_LastUpdate = time(0);
	...
	while(gs_Running)
	{
		...
		for(int i = 0; i < NET_MAX_CLIENTS; i++)
		{
			...
			else
			{
				...
				str_format(pBuf, sizeof(aFileBuf) - (pBuf - aFileBuf), "{ \"name\": \"%s\", \"type\": \"%s\", \"host\": \"%s\", \"location\": \"%s\", \"ip\": \"%s\", \"last_update\": %" PRId64 ", \"online4\": false, \"online6\": false, \"last_network_in\": %" PRId64 ", \"last_network_out\": %" PRId64 " },\n",
					pClients[i].m_aName, pClients[i].m_aType, pClients[i].m_aHost, pClients[i].m_aLocation, pClients[i].m_aIP, pClients[i].m_LastUpdate, pClients[i].m_LastNetworkIN, pClients[i].m_LastNetworkOUT);
				pBuf += strlen(pBuf);
			}
		}
